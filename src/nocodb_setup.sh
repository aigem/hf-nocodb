...
NC_DB="postgres://${DB_USER}:${DB_PASSWORD}@${DB_HOST}:${DB_PORT}/${DB_NAME}"

# 将环境变量写入到 nocodb 用户的主目录
echo "export NC_DB=\"${NC_DB}\"" >> $HOME_DIR/.nocodb_env
echo "export DB_HOST=\"${DB_HOST}\"" >> $HOME_DIR/.nocodb_env
echo "export DB_PORT=\"${DB_PORT}\"" >> $HOME_DIR/.nocodb_env
echo "export DB_USER=\"${DB_USER}\"" >> $HOME_DIR/.nocodb_env
echo "export DB_PASSWORD=\"${DB_PASSWORD}\"" >> $HOME_DIR/.nocodb_env
echo "export DB_NAME=\"${DB_NAME}\"" >> $HOME_DIR/.nocodb_env
